...
import requests  # für HTTPError/429-Detection im Lane-Failover
...
# Dual-VPN-Splitting: Library-Sync ruft Deezer pro File einmal an. Mit alterneirender
# Lane-Zuweisung (file_idx % 2) verteilt sich die Last auf zwei Source-IPs — bei
# großen Libraries (~1000+ Tracks) drückt das Quota-Limit-Hits sichtbar runter.
# Bei VPN_SPLIT_ENABLED=false fällt alles auf "default" zurück (1:1 Status quo).
_VPN_SPLIT_ENABLED = os.environ.get("VPN_SPLIT_ENABLED", "").strip().lower() == "true"
_NAV_LANES: Tuple[str, ...] = ("a", "b") if _VPN_SPLIT_ENABLED else ("default",)
...
def run_navidrome_library_sync(deezer_service: Any, spotify_service: Any = None) -> None:
    ...
    # Per-Lane-Aggregat (nur Deezer; Spotify bleibt auf Default-Routing, weil
    # spotipy keinen Source-Bind-Hook anbietet).
    lane_searched: Dict[str, int] = {l: 0 for l in _NAV_LANES}
    failover_count = 0

    def _deezer_search_with_failover(query: str, lane: str) -> List[Dict[str, Any]]:
        """Search-Wrapper: bei 429 auf der zugewiesenen Lane einmaliger
        Failover auf die andere Lane. Aktualisiert failover_count via nonlocal.
        """
        nonlocal failover_count
        try:
            return deezer_service.search_tracks(query, limit=15, source=lane)
        except requests.HTTPError as e:
            status = getattr(e.response, "status_code", None)
            if status == 429 and lane in _NAV_LANES and len(_NAV_LANES) > 1:
                other = _NAV_LANES[(_NAV_LANES.index(lane) + 1) % len(_NAV_LANES)]
                try:
                    res = deezer_service.search_tracks(query, limit=15, source=other)
                    failover_count += 1
                    return res
                except Exception:
                    return []
    ...
    for idx, path in enumerate(files):
        ...
        # Lane-Round-Robin: gerader Index → Lane A, ungerader → Lane B (oder "default").
        lane = _NAV_LANES[idx % len(_NAV_LANES)]
        lane_searched[lane] += 1

        try:
            hits = _deezer_search_with_failover(f"{artist} {title}", lane)
            did = _best_catalog_id(hits, artist, title)
            if did:
                record_completed_download(did, "deezer")
                deezer_ok += 1
        except Exception as e:
            print(f"Navidrome sync Deezer error ({path.name}, lane {lane}): {e}")
    ...
    if _VPN_SPLIT_ENABLED:
        lane_summary = (
            f", lane_a_searched={lane_searched.get('a', 0)}, "
            f"lane_b_searched={lane_searched.get('b', 0)}, "
            f"failovers={failover_count}"
        )
    else:
        lane_summary = ""
    print(
        f"Navidrome library sync: scanned {len(files)} files, "
        f"Deezer rows {deezer_ok}, Spotify rows {spotify_ok}, "
        f"no tags/filename {failed}{lane_summary}"
    )
